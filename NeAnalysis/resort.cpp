...
const double MACROPULSE_OFFSET = 814; // in ns
// 842 for Sn
...
// for holding event data in the new channel-specific trees
unsigned int evtNo, macroNo;
double completeTime, macroTime;
vector<int> waveform;

// channel-specific trees' event structure (this will allow us to pass events
// into the new channel-specific sub-trees)
struct event
{
...
// buffer for transferring waveform data from raw tree to sub-trees
vector<int> *dummyWaveform;

// to assign the correct macropulses to each event, we will need to look out for
// timestamp resets from DPP/waveform mode changes.
// Use these 'holder' variables to compare consecutive events and identify
// timestamp resets of the detector, monitor, and target changer.
...
void processDPPEvent(TTree* tree)
{
    ...
    // Eliminate events outside the beam window (within 650 microseconds of the
    // beam start signal)
...
void processTargetChanger()
{
    ...
    for (int i=0; i<totalEntries; i++)
    {
        ...
        if (chNo == 0)
        {
            ...
            cout << " macroNo = " << macroNo << ", macroTime = " << macroTime << "\r";
            fflush(stdout);
        }
...
// loop over the raw tree, which contains all events, and assign detector events
...
void processDetEvents()
{
    ...
    // Loop through all channels and pass events from raw tree into sub-trees
    // (2 is monitor, 4 is summed detector)
        ...
        for (int index = 0; index<totalEntries; index++)
        {
            ...
            if (chNo == detIndex)
            {
                ...
                if (evtType == 1)
                {
                    ...
                    {
                        // timestamp-reset failure; log the failure and move on
                    ...
                    else if (completeTime-macroTime-TIME_OFFSET > 8000000)
                    {
                        ...
                        if (evtNo==1)
                        {
                            // new DPP period found because the macropulse has
                            // evtNo==1, so loop through raw tree events to
                            // find the next DPP period
                            do
                            {
                                do
                                {
                        ...
                        else if ((macroTime-prevMacroTime > MACRO_PERIOD-30000 && macroTime-prevMacroTime < MACRO_PERIOD+30000) || (macroTime-prevMacroTime > 2*(MACRO_PERIOD-30000) && macroTime-prevMacroTime < 2*(MACRO_PERIOD+30000)) || (macroTime-prevMacroTime > 3*(MACRO_PERIOD-30000) && macroTime-prevMacroTime < 3*(MACRO_PERIOD+30000)))
                        {
                            // no beam anomaly - keep filling the
                            // channel-specific tree and continue as normal
                        }
                ...
                {
                    // we're in waveform mode - add the event to the waveform tree
...
int main(int argc, char* argv[])
{
    ...
    else
    {
        ...
        if(file->Get("tree"))
        {
            ...
            // each channel.
            cout << "Found a raw event tree; creating channel-specific subtrees." << endl;
