{
    ...
    string exp18OGraphName = "O18+1";
    string lit18OGraphName = "18O(n,tot)+1";
    ...
    if(!litNatOGraph)
    {
        cerr << "Error: failed to open " << litNatOGraphName << " in " << litFile << endl;
        exit(1);
    }

    if(!lit18OGraph)
    {
        cerr << "Error: failed to open " << lit18OGraphName << " in " << litFile << endl;
        exit(1);
    }
    ...
    litNatOGraph->SetLineStyle(0);
    litNatOGraph->SetMarkerColor(kBlack);

    exp18OGraph->SetLineColor(kRed-9);
    exp18OGraph->SetLineWidth(5);
    exp18OGraph->SetLineStyle(0);
    exp18OGraph->SetMarkerColor(kRed-9);

    lit18OGraph->SetLineColor(kGray+1);
    lit18OGraph->SetLineWidth(5);
    lit18OGraph->SetLineStyle(0);
    lit18OGraph->SetMarkerColor(kGray+1);
    ...
    expNatOGraph->GetYaxis()->SetRangeUser(0,6);

    TLatex latex;
    latex.SetNDC();
    latex.SetTextSize(0.05);
    latex.SetTextAlign(13); // align at top
    latex.DrawLatex(0.75,0.42,"+1 barn");
    ...
    legend->AddEntry(exp18OGraph,"H_{2}^{18}O^{exp.} - H_{2}^{lit.}","l");
    legend->AddEntry(lit18OGraph,"{}^{18}O^{lit.} (derived)","l");
    legend->Draw();
}
