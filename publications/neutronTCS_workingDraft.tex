...
their study of \tot\ relative differences in W isotopes, a full optical
model analysis that includes isovector terms is likely required to
reproduce the oscillatory behavior seen in the relative differences.
...
In this section we present pertinent findings from Markov-Chain Monte Carlo
(MCMC)-driven analyses on
...
considerations, there is now a near-unanimous appreciation that nuclear physics
models must report well-founded theoretical errors as part of their
results. In the case of the DOM, this demands an understanding of parameter
covariances in the high-dimensional region surrounding the region of the
global optimum.

To satisfy these demands, we conducted our DOM analysis using the affine-invariant
Markov-Chain Monte Carlo (MCMC) sampling library, emcee \cite{emcee}, that has
enjoyed widespread use in the astrophysics community. Many reviews are available
on both the principles of MCMC \cite{MCMC ref 1, MCMC ref 2} and on emcee
\cite{DFM paper 1, DFM paper 2}. Table \ref{ParameterValues} shows the parameter
definitions and prior distributions used in this analysis. Table \ref{MCMCResults}
shows relevant diagnostic quantities for validating the MCMC posterior sampling
results.
...
Due the approximate nature of any optical potential in representing the true many-body potential
of a nucleus, it should be expected that DOM predictions for nuclear observables
will suffer from some degree of theoretical error. For example, previous
optical model analyses typically easily reproduce low-angle \el\ data taken at low scattering energies
but have challenges at high energies and at backward angles where the predicted cross section may
differ from experimental results by an order of magnitude or more. Accordingly,
model error must be taken into account when determining the optimal parameters.
The choice of model discrepancy for each point
is somewhat subjective and depends on several factors, including the functional form chosen for the
potential, numerical truncations, and the type of datum being calculated. In the
final analysis, we settled on discrepancy terms based on how well preliminary
fits could reproduce differing regions of each data sector, successes and
failures of past optical model analyses, and numerical approximations used
during the calculation of observables. Table \ref{ModelDiscrepancy} shows the model discrepancy terms we used 
for each data sector. In keeping with the Bayesian mindset of MCMC, future
analyses of this type may wish to consider these model discrepancy 
terms as nuisance parameters, each with their own prior, and marginalize over
them when making theoretical predictions.

While the choice of model discrepancy is by its nature subjective, it should be noted that
some reasonable model discrepancy is a vital component of our analysis. 
Failure to include any discrepancy terms may lead to a biased predictions with
an overconfident uncertainty band as shown in Fig. \ref{ModelDiscrepancyFigure}. 
