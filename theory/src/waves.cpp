...
waves::waves(double rho0, double gamma0, int lMaximum0)
...
void waves::CoulombWave()
{
  ...
  double sigma = alpha*((double)lBegin+1./2.) + gamma*(log(beta)-1.)
  ...
  for (int l=lBegin-1;l>=0;l--) 
    {
      sigma -= atan(gamma/(double)(l+1));
      if (l <= lMaximum) Sigma[l] = sigma;
    }
